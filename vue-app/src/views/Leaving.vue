<template>
  <div class="main">
      ...
      <el-aside width="400px">
        ...
        <div class="item" v-for="(item, index) in workerList" :key="index">
          ...
          <div>
            ...
            <div class="d1">电话: {{ displayPhone(item.phone) }}</div>
            <div>
              <el-button
      ...
      <el-main>
        ...
        <div class="item2" v-for="(item, index) in leavingMessageList" :key="index">
          <div style="display: flex">
            ...
            <div>
              <div class="d2">{{ item.user && item.user.nickname ? item.user.nickname : '匿名用户' }}</div>
              <div class="d3">{{ item.leavingMessage.content }}</div>
              <div class="d2">Time: {{ item.leavingMessage.createAt }}</div>
            </div>
          </div>
          <div v-if="hasReply(item.leavingMessage)" class="reply">
            <div class="d2 reply-label">客服回复</div>
            <div class="d3 reply-text">{{ item.leavingMessage.reply }}</div>
          </div>
        </div>
    ...
    <el-dialog title="客服评价" v-model="dialogFormVisible1">
    ...
    <el-dialog title="添加留言" v-model="dialogFormVisible2">
    ...
    <el-dialog title="我的评价" v-model="dialogFormVisible3">
    ...
    submitLeaving() {
      CreateLeavingMessage(this.leavingMessage).then(res => {
        if (res.success) {
          ...
          ListAllLeavingMessage().then(r => {
            if (r.success) this.leavingMessageList = r.data
          })
        }
      })
    },
